...
ExecStart=$WorkingDirectory/$ENV_NAME/bin/gunicorn --access-logfile $WorkingDirectory/log/access_file_g.log --error-logfile $WorkingDirectory/log/error_file_g.log --capture-output --enable-stdio-inheritance  --workers=3 --max-requests=2 --timeout=120 --bind unix:/run/logodetect.sock logo_detect_api.wsgi:application
